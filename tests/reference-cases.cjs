...
test('reference data validates; all reviewed entries have fixed public source and finite values or null',()=>{
 ...
 assert.deepEqual({...JSON.parse(read('data/reference-stats.json')),acquisition:JSON.parse(read('data/acquisition.json'))},JSON.parse(read('dist/reference-stats-1.4.5.json')));
});
...
test('loader preserves an existing same-version UI and has matching integrity for the versioned script',()=>{
 const l=loader();let shown=0;l.window.__wonderDeck={version:'1.4.5',show:()=>shown++};l.run();assert.equal(shown,1);assert.equal(l.script,undefined);
 delete l.window.__wonderDeck;l.run();
 assert.equal(l.script.src,'https://satokoyo.github.io/wlw/wonder-deck-1.4.5.js');
 assert.equal(l.script.integrity,'sha384-'+crypto.createHash('sha384').update(read('dist/wonder-deck-1.4.5.js')).digest('base64'));
